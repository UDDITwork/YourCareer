...
export function AppHeader({ children, showUpgradeButton = true, isProPlan = false }: AppHeaderProps) {
  ...
  return (
    <header className="h-14 border-b border-black/50 bg-white fixed top-0 left-0 right-0 z-40 font-['Times_New_Roman',_Times,_serif]">
      {/* Content Container */}
      <div className="max-w-[2000px] mx-auto h-full px-3 flex items-center justify-between relative">
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-shrink">
          ...
          <div className="h-5 w-px bg-black/30 hidden sm:block flex-shrink-0" />
            ...
            <>
              ...
              <nav className="hidden md:flex items-center gap-1 lg:gap-2">
                {showUpgradeButton && (
                  <>
                    <ProUpgradeButton />
                    <div className="h-4 w-px bg-black/30 ml-2 lg:ml-3" />
                  </>
                )}
                ...
                <div className="flex items-center px-2 lg:px-3 py-1">
                  ...
                  <div className="mx-1 lg:mx-2 h-4 w-px bg-black/30" />
                  <SettingsButton />
                  <div className="mx-1 lg:mx-2 h-4 w-px bg-black/30" />
                  <LogoutButton />
                </div>
              ...
              <Sheet open={isOpen} onOpenChange={setIsOpen}>
                <SheetTrigger asChild className="md:hidden">
                  <Button variant="ghost" size="icon" className="h-9 w-9 rounded-none text-black">
                    <Menu className="h-5 w-5" />
                  </Button>
                </SheetTrigger>
                <SheetContent side="right" className="w-[280px] sm:w-[320px] border-l border-black/50 bg-white">
                  <SheetHeader>
                    <SheetTitle className="uppercase tracking-wider text-black">Menu</SheetTitle>
                  </SheetHeader>
                  <div className="flex flex-col gap-4 pt-6">
                    ...
                    <Link
                      ...
                      className={cn(
                        ...
                        "text-sm font-medium text-black/80 hover:text-black",
                        "hover:bg-gray-50 transition-colors duration-200 uppercase tracking-wide"
                      )}
                    >
                      <User className="h-4 w-4" />
                      Profile
